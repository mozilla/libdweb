...
var sendChat = async ev => {
  ...
  // multicasting
  await browser.UDPSocket.addMembership(client, multicastGroup)
  const multi_message = encoder.encode(payload).buffer
  await browser.UDPSocket.send(client, multicastGroup, 41235, multi_message)
}
...
var serve = async () => {
  ...
  browser.UDPSocket.setMulticastInterface(server, multicastGroup)
  browser.UDPSocket.addMembership(server, multicastGroup)
  console.log(`udp listening ${server.address.host}:${server.address.port}`)
