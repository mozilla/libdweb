...
const { mm, appinfo } = Cu.import("resource://gre/modules/Services.jsm", {})
...
if (isParent) {
  mm.loadProcessScript(`data:,Components.utils.import("${__URI__}", {})`, true)
      ...
      return {
        protocol: {
          installProtocol(scheme, uuid) {
            install(scheme, uuid)
            mm.broadcastAsyncMessage(`${__URI__}::install`, { scheme, uuid })
          }
        }
      }
    }
  ...
  mm.addMessageListener(`${__URI__}::install`, {
    receiveMessage({ data: { scheme, uuid } }) {
      install(scheme, uuid)
    }
  })
}
